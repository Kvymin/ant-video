...
import { dataDir, mrefreshToken, host, ignoreCategory, token, userId } from "../config.js"
...
async function updateTV(_hours) {
  ...
  interfacePath = `${dataDir}/interface.txt.bak`
  // txt
...
async function updatePE(_hours) {
  ...
  const interfacePath = `${dataDir}/interface.txt.bak`
  const interfaceTXTPath = `${dataDir}/interfaceTXT.txt.bak`

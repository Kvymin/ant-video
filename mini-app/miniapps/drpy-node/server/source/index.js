...
const MINIAPP_MODE = process.env.DRPY_MINIAPP === '1';
const PORT = Number(process.env.DEV_HTTP_PORT || 5757);
const WsPORT = Number(process.env.DRPY_WS_PORT || 57575);
...
fastify.addHook('onReady', async () => {
    if (!MINIAPP_MODE) await checkPhpAvailable();
...
// 宿主 Node 小程序的就绪探针。该路由不需要任何业务配置或鉴权。
if (MINIAPP_MODE) {
    fastify.get('/check', async () => ({ok: true, service: 'drpy-node'}));
    fastify.get('/miniapp/status', async () => ({ok: true, service: 'drpy-node', version: '1.4.7'}));
}
...
const start = async () => {
    try {
        ...
        if (!MINIAPP_MODE) await startWebSocketServer({port: WsPORT, host: '::'});
...
const stop = async () => {
    try {
        // 停止 WebSocket 服务器
        if (!MINIAPP_MODE) await stopWebSocketServer();
        // 停止主服务器
        await fastify.server.close();
...
if (currentFile === indexFile && !process.env.DRPY_NO_AUTOSTART) {
    start();
}

...
function initializeFastify() {
    ...
    // 宿主 Node worker 注入 catServerFactory，用于感知真实监听端口。
    // 不传时保持原项目的 Fastify 默认 HTTP server 行为，方便独立运行。
    const fastifyOptions = { logger: _logger };
    if (typeof globalThis.catServerFactory === 'function') {
        fastifyOptions.serverFactory = globalThis.catServerFactory;
    }
    const _fastify = Fastify(fastifyOptions);
